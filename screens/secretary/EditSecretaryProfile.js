...
const EditSecretaryProfile = (props) => {
    ...
    const [email, setEmail] = useState(authRed.user.email)
    const [name, setName] = useState(authRed.name)
    const [phoneNumber, setPhoneNumber] = useState(authRed.user.phone)
    const [city, setCity] = useState(authRed.user.city || '')
    const [workField, setWorkField] = useState(authRed.user.work || authRed.workField)
    const [title, setTitle] = useState(authRed.user.title || authRed.title)
